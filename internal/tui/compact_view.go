...
func (m Model) renderCompactRows(s *strings.Builder, rowIndexes []int, accountWidth int) {
	...
	for _, i := range rowIndexes {
		...
		row := m.renderCompactAccountRow(i, m.Accounts[i], accountWidth)
...
func (m Model) renderCompactAccountRow(index int, acc management.Account, accountWidth int) string {
	var s strings.Builder
	...
	name := truncateLabel(accountLabel(acc), nameWidth-1)
	...
	if err := m.ErrorsMap[acc.ID]; err != nil {
	...
	windows, ok := m.UsageData[acc.ID]
	if !ok {
		s.WriteString(m.renderCompactStatusRow("Queued...", barWidth, percentWidth, resetWidth))
		return s.String()
	}
	window := primaryQuotaWindow(windows)
	if !window.HasPercent {
		s.WriteString(m.renderCompactStatusRow("No quota data", barWidth, percentWidth, resetWidth))
		return s.String()
	}

	ratio := m.compactBarRatio(acc.ID, clampRatio(window.LeftPercent/100))
	s.WriteString(renderSmoothBar(barWidth, ratio, defaultBarGradientStart, defaultBarGradientEnd))
	s.WriteString(" ")
	s.WriteString(m.renderCompactPercent(fmt.Sprintf("%.0f%%", window.LeftPercent), percentWidth))
...
func (m Model) isCompactAccountExhausted(accountKey string) bool {
	...
	if account := m.findAccountByID(accountKey); account != nil {
		switch account.State {
		case "cooling_down", "needs_reauth", "paused":
			return true
		}
	}
	...
	windows, ok := m.UsageData[accountKey]
	if !ok {
		return false
	}
	return isConfirmedExhausted(primaryQuotaWindow(windows))
}

func isConfirmedExhausted(window quotaWindow) bool {
	if !window.HasPercent {
		return false
	}
...
func isConfirmedNonExhausted(window quotaWindow) bool {
	if !window.HasPercent {
		return false
	}
	return clampRatio(window.LeftPercent/100) > 0
...
func (m Model) renderCompactStatusRow(status string, barWidth, percentWidth, resetWidth int) string {
	row := renderSmoothBar(barWidth, 0, defaultBarGradientStart, defaultBarGradientEnd)
	row += " "
	row += m.renderCompactPercent("...", percentWidth)
	if resetWidth > 0 {
...
func (m Model) renderCompactPercent(value string, width int) string {
