...
// argv assembles the exact process argv, applying force where the method
// honors it (script reruns are idempotent, so scripts never take force args).
func (p Plan) argv(force bool) []string {
	if p.Script != nil {
		return append([]string{p.Script.Interpreter}, p.Script.URL)
	}
