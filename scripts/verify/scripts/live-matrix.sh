...
EVID_FINAL="${PRISM_VERIFY_EVIDENCE_DIR:-$REPO_ROOT/scripts/verify/evidence/live-matrix/$RUN_ID}"
...
for ID in grok omp; do
  node "$REPO_ROOT/scripts/verify/scripts/apply-integration.mjs" "$WS" "$ID" \
...
node "$REPO_ROOT/scripts/verify/scripts/assert-matrix-run.mjs" "$EVID_WORK" >/dev/null \
