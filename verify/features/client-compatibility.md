...
- Live matrix: for each of `codex/gpt-5.6-luna`, `antigravity/gemini-3.7-flash`, and one chat-wire model taken from the daemon's model list at runtime, Grok and OMP each hold one session alive >=300 seconds with exit 0, a substantive final response, and zero transport errors.
- New-agent smoke: Claude, Pi, OpenCode, and Hermes each answer one live request through the config written by Integrations. Use `opencode run --standalone --model prism/codex/gpt-5.6-luna ...` in an isolated home. Require exit 0 and a substantive response.

## How to get to it (user POV)

Click Apply for Grok and OMP in Prism. Start each client normally, select its Prism model — for the matrix models Grok uses `prism-codex-gpt-5-6-luna`, `prism-antigravity-gemini-3-7-flash`, and the derived chat model alias, and OMP uses `prism/codex/gpt-5.6-luna`, `prism/antigravity/gemini-3.7-flash`, and the derived chat model path — and send a prompt that requests reasoning. The request must finish. OMP must render thinking before the final answer.
