...
if [ "$LEGACY" = "1" ]; then
  cp "$REPO_ROOT/scripts/verify/fixtures/grok-legacy-prism.toml" "$RUNDIR/.grok/config.toml"
fi
...
WS=$(node "$REPO_ROOT/scripts/verify/scripts/cdp-ws.mjs" "$CDP_PORT") || fail "no Electron CDP page target"

cdp_eval() {
  node "$REPO_ROOT/scripts/verify/scripts/cdp-eval.mjs" "$WS" "$1"
}
...
for ID in codex grok omp claude pi opencode opencode2 hermes; do
  if ! click_apply "$ID" > "$EVID_WORK/apply-$ID.json" 2>&1; then
    ...
    node "$REPO_ROOT/scripts/verify/scripts/cdp-screenshot.mjs" "$WS" "$EVID_WORK/integrations-failure.png" || true
    fail "UI Apply failed for $ID"
  fi
done

cdp_eval "document.querySelector('main')?.innerText" > "$EVID_WORK/integrations-after.json"
node "$REPO_ROOT/scripts/verify/scripts/cdp-screenshot.mjs" "$WS" "$EVID_WORK/integrations.png"
...
node "$REPO_ROOT/scripts/verify/scripts/cdp-screenshot.mjs" "$WS" "$EVID_WORK/integrations-cards.png"
for ID in codex grok omp claude pi opencode opencode2 hermes; do
...
node "$REPO_ROOT/scripts/verify/scripts/cdp-screenshot.mjs" "$WS" "$EVID_WORK/integrations-after-rollback.png"
...
node "$REPO_ROOT/scripts/verify/scripts/cdp-screenshot.mjs" "$WS" "$EVID_WORK/integrations-after-bulk.png"
...
if [ "$LIVE" = "1" ]; then
  ...
  if ! node "$REPO_ROOT/scripts/verify/scripts/assert-omp-output.mjs" \
