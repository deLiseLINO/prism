...
function MachineRow({ host, generation, onChanged }: MachineRowProps): JSX.Element {
  ...
  async function remove(): Promise<void> {
    const result = await task.run(() => api.deleteHost(host.id, generation))
  ...
  return (
    <div className="int-row-wrap">
      ...
      {task.error === null && actionError !== null ? (
        <div className="int-refusal" role="alert">
...
export function MachinesView(): JSX.Element {
  ...
  async function add(): Promise<void> {
    setAddError(null)
    const result = await addTask.run(() => api.createHost({
      id: id.trim(),
      ...
      >
        {(list) => (
          <div className="int-list" style={{ '--i': 2 } as CSSProperties}>
            {list.map((host) => (
              <MachineRow key={host.id} host={host} generation={gen} onChanged={() => { hosts.refresh(); generation.refresh() }} />
            ))}
          </div>
        )}
      </AsyncBoundary>
