...
EVID_FINAL="${PRISM_VERIFY_EVIDENCE_DIR:-$REPO_ROOT/scripts/verify/evidence/vision-sidecar/$RUN_ID}"
...
cdp_eval() {
  node "$REPO_ROOT/scripts/verify/scripts/cdp-eval.mjs" "$WS" "$1" 90000
}
...
node "$REPO_ROOT/scripts/verify/scripts/cdp-screenshot.mjs" "$CDP_PORT" "$EVID_WORK/sidecar-card.png" > /dev/null 2>&1 || true
