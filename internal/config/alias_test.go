...
func TestPrismAliasRoundTrip(t *testing.T) {
	a, err := PrismAlias("Grok Build CLI")
	if err != nil {
		t.Fatal(err)
	}
	if a != "prism-grok-build-cli" {
		t.Fatalf("got %q", a)
	}
	name, err := ParsePrismAlias(a)
	if err != nil {
		t.Fatal(err)
	}
	a2, err := PrismAlias(name)
	if err != nil {
		t.Fatal(err)
	}
	if a2 != a {
...
func TestClaudeAliasRoundTrip(t *testing.T) {
	...
	if a != "claude-prism-codex-main--gpt-5-2-codex" {
		t.Fatalf("got %q", a)
	}
...
func TestAliasBuildersRejectEmptySanitizedParts(t *testing.T) {
	if _, err := PrismAlias("---"); !errors.Is(err, ErrMalformedAlias) {
...
func TestAliasParsersRejectForeignFamilies(t *testing.T) {
	if _, err := ParsePrismAlias("claude-prism-p--m"); !errors.Is(err, ErrMalformedAlias) {
		t.Fatalf("want ErrMalformedAlias, got %v", err)
	}
	if _, _, err := ParseClaudeAlias("prism-p"); !errors.Is(err, ErrMalformedAlias) {
		t.Fatalf("want ErrMalformedAlias, got %v", err)
	}
	if _, err := ParsePrismAlias("prism-not--sanitized"); !errors.Is(err, ErrMalformedAlias) {
		t.Fatalf("want ErrMalformedAlias, got %v", err)
	}
	if _, _, err := ParseClaudeAlias("claude-prism-pmodel"); !errors.Is(err, ErrMalformedAlias) {
		t.Fatalf("want ErrMalformedAlias, got %v", err)
	}
}
