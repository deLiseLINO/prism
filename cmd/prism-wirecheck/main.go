...
func main() {
	...
	caseID := flag.String("case", "", "run one case by id; empty runs the responses-core, chat-core, mcp-core, vision-core and reasoning-core suites")
	...
	for i := range authority.Cases {
		...
		if c.Suite == "responses-core" || c.Suite == "chat-core" || c.Suite == "mcp-core" || c.Suite == "vision-core" || c.Suite == "reasoning-core" {
			selected = append(selected, *c)
		}
	}
	if len(selected) == 0 {
