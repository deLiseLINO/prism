...
func TestTurnRunErrorCausePassesThroughAsFailureMessage(t *testing.T) {
	cause := errors.New("openai: 400 unknown parameter 'verbosity'")
	runErr := provider.RunError{Kind: provider.TerminalOmitted, Class: provider.ClassInvalidRequest, ReplaySafe: true, Cause: cause}
	j := requestlog.New(8, time.Now)
	pool := poolWith("codex", 1)
	runners := fakeRunners{"codex": failRunner(runErr)}
	planner := singlePlan("codex", 0, TurnPolicy{})
	...
	if f.Event.Failure.Reason != canon.FailInvalidRequest {
		t.Fatalf("failure reason = %v, want invalid_request", f.Event.Failure.Reason)
	}
	if f.Event.Failure.Message != cause.Error() {
		t.Fatalf("failure message = %q, want upstream cause %q", f.Event.Failure.Message, cause.Error())
	}
	snap := j.Snapshot()
	if len(snap) != 1 || len(snap[0].Attempts) != 1 {
		t.Fatalf("journal = %+v, want a single attempt", snap)
	}
	if snap[0].Attempts[0].Error != cause.Error() {
		t.Fatalf("journal attempt error = %q, want upstream cause %q", snap[0].Attempts[0].Error, cause.Error())
	}
}

func TestTurnPointerRunErrorMapsClassAndPassesCauseThrough(t *testing.T) {
	cause := errors.New("anthropic: This model is overloaded (type=rate_limit_error status=429)")
	...
	if f.Event.Failure.Reason != canon.FailRateLimited {
		t.Fatalf("failure reason = %v, want rate_limited", f.Event.Failure.Reason)
	}
	if f.Event.Failure.Message != cause.Error() {
		t.Fatalf("failure message = %q, want upstream cause %q", f.Event.Failure.Message, cause.Error())
	}
