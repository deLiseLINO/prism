...
func newAuthFlowEnv(t *testing.T, accessSecret string) *authFlowEnv {
	...
	srv := New(pool, cfg, &fakeCatalog{models: []provider.Model{{ID: "gpt-5.3"}}},
		&fakeQuotaSource{snapshots: map[account.AccountID]quota.Snapshot{}}, credsAdapter, svc, integrations.NewRegistry(), nil, nil)
...
func TestAuthFlowWireAliasResolvesConfiguredProvider(t *testing.T) {
	...
	srv := New(pool, cfg, &fakeCatalog{models: []provider.Model{{ID: "gpt-5.2"}}},
		&fakeQuotaSource{snapshots: map[account.AccountID]quota.Snapshot{}}, &fakeCreds{store: map[string][]byte{}}, svc, integrations.NewRegistry(), nil, nil)
	h := srv.Handler()
