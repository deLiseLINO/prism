...
func TestResolveInstallPlanPerToolAvailability(t *testing.T) {
	cases := []struct {
		...
		{name: "codex darwin script last", key: "codex", goos: "darwin", tools: nil, wantOK: true, wantMethod: MethodScript, wantCommand: "bash https://chatgpt.com/codex/install.sh"},
		...
		{name: "grok script first", key: "grok", goos: "darwin", tools: []string{"npm", "brew"}, wantOK: true, wantMethod: MethodScript, wantCommand: "bash https://x.ai/cli/install.sh"},
...
// TestPlanArgvForce asserts force flags append only where honored: npm gets
// --force, brew gets --force, scripts rerun identically.
